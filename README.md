...
Token is used for authentication of the registered user.

- [ ] What steps can you take in your web apps to keep your data secure?

In React environment, use private routes and Switch component of react-router so only the authenticated users can see the regarding pages.

- [ ] Describe how web servers work.

Client makes a request from a site through clicking their url, the request is directed to the correct server by the DNS of the domain name, the server reads what is requested and respond accordingly. Server-side rendering is where HTML of the page is returned by the server whereas in client-side rendering (like we do in React) the request to the server returns only the required data and our React app renders the pages on the DOM which saves time and energy.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

(C)reate - POST method
(R)ead - GET method
(U)pdate - PUT method (some suggest PATCH)
